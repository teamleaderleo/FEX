...
def parse_callbacks(text: str):
    ...
    for line in text.splitlines():
        ...
        hash_bytes = match.group(3)
        key = signature
        if key in seen:
            continue
        seen.add(key)
        callbacks.append((index, signature, hash_bytes))
    if not callbacks:
...
def write_bridge(path: Path, callbacks, prefix: str):
    lines = ["// Generated by extract_guest_bridge.py. Do not edit.", ""]
...
def write_accessors(path: Path, callbacks, prefix: str):
    lines = [
        ...
        "#include <cstdint>",
        "",
        "template<typename Signature> struct FEXResidentBridgeInvoker;",
        "template<typename Signature> struct FEXResidentBridgeUnpacker;",
        "",
    ]
    for index, signature, _ in callbacks:
        lines += [
...
def main():
    ...
    parser.add_argument("--prefix", default="libcuda")
    ...
    print(f"extracted {len(callbacks)} unique resident bridge signatures for {prefix}")
