...
static void *VKAPI_CALL allocation_cb(void *user, size_t size, size_t alignment, VkSystemAllocationScope scope) {
  ...
  fprintf(stderr, "ALLOC_CB ptr=%p size=%zu alignment=%zu count=%u\n", p, size, alignment, alloc_calls);
  return p;
}
...
static void *VKAPI_CALL reallocation_cb(void *user, void *original, size_t size, size_t alignment, VkSystemAllocationScope scope) {
  ...
  fprintf(stderr, "REALLOC_CB_UNEXPECTED\n");
  return NULL;
}
...
static void VKAPI_CALL free_cb(void *user, void *memory) {
  ...
  ++free_calls;
  fprintf(stderr, "FREE_CB_ENTER ptr=%p count=%u\n", memory, free_calls);
  /* Deliberately leak. The discriminator is callback transition, not guest libc free. */
  fprintf(stderr, "FREE_CB_RETURN ptr=%p\n", memory);
}
...
int main(void) {
  ...
  VkApplicationInfo app = {.sType=VK_STRUCTURE_TYPE_APPLICATION_INFO,.pApplicationName="generated-allocator",.apiVersion=VK_API_VERSION_1_0};
  ...
  uint32_t family=0; while(family<nq&&!qp[family].queueCount)++family; if(family==nq)return 8;
  float pri=1.0f;
  ...
  fprintf(stderr, "CREATE_ENTER callbacks=%p/%p/%p user=%p\n", (void*)cb.pfnAllocation, (void*)cb.pfnReallocation, (void*)cb.pfnFree, cb.pUserData);
  r=vkCreateBuffer(device,&bci,&cb,&buffer);
  fprintf(stderr, "CREATE_RETURN result=%d buffer=%p alloc=%u free=%u\n", r, (void*)(uintptr_t)buffer, alloc_calls, free_calls);
  if(r!=VK_SUCCESS)return 10;
  fprintf(stderr, "DESTROY_ENTER buffer=%p\n", (void*)(uintptr_t)buffer);
  vkDestroyBuffer(device,buffer,&cb);
  fprintf(stderr, "DESTROY_RETURN alloc=%u free=%u\n",alloc_calls,free_calls);

  vkDestroyDevice(device,NULL); vkDestroyInstance(instance,NULL); free(qp); free(phys);
  if (!alloc_calls || !free_calls) return 11;
  fprintf(stderr, "PASS generated allocation callbacks crossed host boundary\n");
  return 0;
}
