...
# Test-only synchronous retained-listener discriminator.
# Generation 1 registers a listener. A custom thunked trigger invokes the
# retained host trampoline on the current FEX thread before close and again
# from generation 2 after a forced moved reload, without re-registering.
...
'''
new_return = '''  // Linux Fieldwork diagnostic: retain the already-finalized FEX trampoline.
  fex_wayland_retained_test_callback = reinterpret_cast<FEXWaylandTestCallback>(callback_table[0]);
...
extern "C" void fexfn_impl_libwayland_client_fex_wl_test_trigger(uint32_t value) {
  ...
  fflush(stderr);
  if (!fex_wayland_retained_test_callback) {
    std::abort();
  }
