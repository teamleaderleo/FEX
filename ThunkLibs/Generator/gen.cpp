...
void GenerateThunkLibsAction::OnAnalysisComplete(clang::ASTContext& context) {
  ...
  if (!output_filenames.guest_bridge.empty()) {
    ...
    fmt::print(file,
               "extern \"C\" __attribute__((visibility(\"default\"))) uintptr_t fex_bridge_{}_invoker_lookup(uint32_t index) {{\n",
               libname);
    file << "  switch (index) {\n";
    for (const auto& entry : guest_funcptr_entries) {
      fmt::print(file, "  case {}: return reinterpret_cast<uintptr_t>(GetCallerForHostFunction((fex_bridge_signature_{}*)nullptr));\n",
                 entry.index, entry.index);
    }
    file << "  default: __builtin_trap();\n"
            "  }\n"
            "}\n\n";

    if (std::ranges::any_of(guest_funcptr_entries, [](const auto& entry) { return entry.used_as_callback; })) {
      fmt::print(file,
                 "extern \"C\" __attribute__((visibility(\"default\"))) uintptr_t fex_bridge_{}_unpacker_lookup(uint32_t index) {{\n",
                 libname);
      file << "  switch (index) {\n";
      for (const auto& entry : guest_funcptr_entries) {
        if (entry.used_as_callback) {
          fmt::print(file, "  case {}: return reinterpret_cast<uintptr_t>(CallbackUnpack<fex_bridge_signature_{}>::Unpack);\n", entry.index,
                     entry.index);
        }
      }
      file << "  default: __builtin_trap();\n"
  ...
  if (!output_filenames.guest_bridge_accessors.empty()) {
    ...
    fmt::print(file, "extern \"C\" uintptr_t fex_bridge_{}_invoker_lookup(uint32_t);\n", libname);
    if (std::ranges::any_of(guest_funcptr_entries, [](const auto& entry) { return entry.used_as_callback; })) {
      fmt::print(file, "extern \"C\" uintptr_t fex_bridge_{}_unpacker_lookup(uint32_t);\n", libname);
    }
    file << "\n";
    ...
    for (const auto& entry : guest_funcptr_entries) {
      ...
      fmt::print(file, "    return reinterpret_cast<fex_bridge_accessor_signature_{}*>(fex_bridge_{}_invoker_lookup({}));\n", entry.index,
                 libname, entry.index);
      file << "  }\n};\n";
      if (entry.used_as_callback) {
        ...
        fmt::print(file, "    return reinterpret_cast<void (*)(uintptr_t, void*)>(fex_bridge_{}_unpacker_lookup({}));\n", libname,
                   entry.index);
        file << "  }\n};\n";
      }
      file << "\n";
