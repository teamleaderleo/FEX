...
int main(int argc, char** argv) {
  ...
  std::fprintf(stderr, "INFLIGHT relink-reset H=%p T=%p sentinel=111\n",
  ...
  std::fprintf(stderr, "INFLIGHT replacement-committed H=%p T=%p sentinel=222 reregister=%d\n",
               reinterpret_cast<void*>(SyntheticH), Target, Reregister ? 1 : 0);

  if (Reregister) {
    LinkAddressToFunction(SyntheticH, reinterpret_cast<uintptr_t>(Target));
    std::fprintf(stderr, "INFLIGHT reregistered H=%p T=%p\n",
                 reinterpret_cast<void*>(SyntheticH), Target);
  }
