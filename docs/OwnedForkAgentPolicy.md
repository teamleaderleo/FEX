...
This is the task-selected policy preserved from the former automatic `AGENTS.md`. The root router owns the universal research-authority, provenance, architecture, worktree, and external-interaction boundaries. Load this file when a task needs the complete CI or upstream-preparation rationale.
