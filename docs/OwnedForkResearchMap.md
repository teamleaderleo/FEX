...
| A block-level Fossilize cache lookup is corrupt, slow or confused with whole-file caching | per-block bucket/key identity, bounded blob/index layout and live publication | format-3 blobs are bounded before spans and torn writable index suffixes resume at the valid prefix; allocation/runtime measurement remains separate | [Block-level Fossilize disk cache](BlockDiskCache.md) |
