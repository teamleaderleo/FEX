...
| A block-level Fossilize cache lookup is corrupt, slow or confused with whole-file caching | per-block bucket/key identity, bounded blob layout and live publication | format-3 layout is bounded before spans; allocation/runtime measurement remains separate | [Block-level Fossilize disk cache](BlockDiskCache.md) |
